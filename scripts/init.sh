...
# Each hook is a single file dispatcher that runs every executable 
# inside a sibling <hookname>.d folder, in sort order. The prefixes below are sort order: 
# LFS has to restore file contents before Git Store Meta stamps the timestamps back on. 
# Two digits, with gaps, so parts can be slotted in between later.
LFS_PART="10-git-lfs"
META_PART="20-git-store-meta"
# Marker file that indicates which version of each tool was used, so that a
# part is only regenerated locally by newer versions.
...
# identically whether bash is in POSIX/sh mode or not, unlike echo builtin.
#
...
# so the match is anchored on the git-lfs/ prefix.
#
# Prints nothing when the format is unrecognized.
...
function init_lfs () {
  # Initialize Git LFS smudge filters. Actual hooks are handled by bootstrap_hooks.
  # --skip-repo: used to only write the global clean/smudge/process filter config.
...
# Location where both generators are made to write, before their output is harvested
# into .githooks. NOT `git rev-parse --git-path hooks`, which resolves through
# core.hooksPath and would answer ".githooks" on a re-run.
...
#
# That works so long as the hook is located at .githooks/<name>. A part at
# .githooks/<name>.d/<part> is one directory deeper, so it misses.
...
# Writes the dispatch loop shared by every generated hook, so a repo with five
# hooks carries one copy of the loop rather than five. Exec'd by each stub as:
#
#   dispatcher-common.sh <hook> <abort_on_error> <stdin_sensitive> -- "$@"
#
# The body is copied out verbatim -- no substituted values -- so it is
# byte-identical every run and shared, unmodified, by every hook.
function write_dispatcher_common () {
  local hooks_dir=$1
  local common="${hooks_dir}/dispatcher-common.sh"

  cat <<'EOF' > "${common}" || return 1
#!/bin/sh
...
EOF
  ...
  chmod a+x "${common}" || return 1
}

# Writes the small per-hook stub that execs dispatcher-common.sh with this
# hook's name and flags. The output is byte-identical every run -- no
# timestamps, no version strings -- so re-running init.sh leaves git status
# clean.
function write_dispatcher () {
  ...
  write_dispatcher_common "${hooks_dir}" || return 1

  # $hook, $abort_on_error and $stdin_sensitive are substituted here, at
  # generation time; $0 and $@ are escaped so they resolve at hook run time,
  # inside dispatcher-common.sh.
  cat <<EOF > "${dispatcher}" || return 1
#!/bin/sh
# Generated by git-logic-init -- do not edit.
exec "\$(dirname "\$0")/dispatcher-common.sh" ${hook} ${abort_on_error} ${stdin_sensitive} -- "\$@"
EOF
...
function bootstrap_hooks () {
  ...
  # everyone through an ordinary commit, and older machines leave it alone.
